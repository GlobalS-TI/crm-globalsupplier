          ...
          <div className="px-4 pb-4 flex items-center justify-center">
            {kind === 'image' && (
              <img
                src={url}
                alt={name}
                className="max-h-[75vh] max-w-full object-contain rounded-md bg-muted/30"
              />
            )}
            {kind === 'video' && (
              <video src={url} controls autoPlay className="max-h-[75vh] max-w-full rounded-md bg-black" />
            )}
            {kind === 'pdf' && (
              <iframe src={url} title={name} className="w-full h-[75vh] rounded-md border" />
            )}
          </div>
